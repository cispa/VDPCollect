...
async function collectStorage() {
    console.log("Starting collectStorage...");
    ...
    // Collect cookies from the background script
    if (document.cookie !== "") {
    ...
    console.log("Collected localStorage:", storage.storage);
    ...
    console.log("Collected sessionStorage:", storage.storage);

    console.log("Final collected storage:", storage);
    return storage;
}

async function rewriteTaintReport(report) {

    // Filter out extension-generated taint flows
    if (isExtensionUrl(report.detail.stack.source)) {
        console.log("Skipping rewriteTaintReport because it comes from extension");
        return;
    }

    console.log("Rewrite TaintReport started");
    var storage = await collectStorage();

    console.log("Creating flow object");
    var flow = {
        detail: {
    ...
    const reportTaintStr = report.detail.str.taint || ""; 
    console.log("reportTaintStr");
    console.log(reportTaintStr);

    console.log("source");
    console.log(report.detail.stack.source);


    const taintData = {
        url: window.location.href,
        details: report.detail,
        taintFlows: reportTaintStr, 
        source: report.detail.stack.source,
        sink: report.detail.sink,
